...
export const login = (email, password) => async (dispatch) => {
	...
	try {
		...
		localStorage.setItem('userInfo', JSON.stringify(data)); //user credentials store in local storage even after refresh
	} catch (error) {
		dispatch(
			setError(
...
export const logout = () => (dispatch) => {  
	localStorage.removeItem('userInfo');    // erase user credentials after user logout
...
export const register = (name, email, password) => async (dispatch) => {
	...
	try {
		...
		dispatch(userLogin(data));  //after registration we want user to login
...
export const googleLogin = (googleId, email, name, googleImage) => async (dispatch) => {
	...
	try {
		...
		dispatch(userLogin(data));
		localStorage.setItem('userInfo', JSON.stringify(data)); //incase user refresh the page, store in local storage
	} catch (error) {
		dispatch(
			setError(
				error.response && error.response.data.message
